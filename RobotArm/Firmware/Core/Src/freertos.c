...
TaskHandle_t gListenerTaskHandle = NULL;
...
void MX_FREERTOS_Init(void) {
  ...
  osThreadDef(ListenerTask, Listener, osPriorityAboveNormal, 0, 128);
  ...
  osThreadDef(PresenterTask, Presenter, osPriorityNormal, 0, 256);
...
void Listener(void const * argument)
{
  /* USER CODE BEGIN Listener */
	gListenerTaskHandle = xTaskGetCurrentTaskHandle();
	Listener_Init();
  /* Infinite loop */
  for(;;)
  {
